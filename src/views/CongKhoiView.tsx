...
import {
  CONGKHOI_LEVELS,
  CONGKHOI_PASS_THRESHOLD,
  QUESTIONS_PER_CONGKHOI_LEVEL,
  generateCongKhoiDeck,
  isCongKhoiLevelUnlocked,
  type CongKhoiLevel,
  type CongKhoiQuestion,
} from '../data/congKhoiData';
...
const STORAGE_KEY = 'lingoland_congkhoi_passed';
...
// Đơn vị chiều cao 1 ô khối. Tổng max = 10 → tháp max 10×28 = 280px.
const UNIT_HEIGHT = 28;
const BLOCK_WIDTH = 56;

// Render 1 khối có giá trị value → rectangle màu theo BLOCK_STYLES.
  ...
  return (
    <div
      ...
      style={{
        ...
        fontSize,
        // Bo góc trên + dưới nhẹ — khi xếp chồng vẫn liền mạch.
        borderRadius: 6,
      }}
    >
      {value}
...
type CongKhoiViewProps = {
  onBack: () => void;
};

export default function CongKhoiView({ onBack }: CongKhoiViewProps) {
  const [phase, setPhase] = useState<Phase>('idle');
  const [activeLevel, setActiveLevel] = useState<CongKhoiLevel | null>(null);
  const [deck, setDeck] = useState<CongKhoiQuestion[]>([]);
  ...
  // Voice đọc câu hỏi mỗi khi câu mới.
  useEffect(() => {
    if (phase !== 'playing' || !q) return;
    const t = window.setTimeout(() => {
      speak(`${q.a} cộng ${q.b} bằng mấy?`, LANG_SPEAK_DEFAULT);
    }, 400);
  ...
  useEffect(() => {
    ...
    inactiveRef.current = window.setTimeout(() => {
      if (q) speak(`${q.a} cộng ${q.b} bằng mấy?`, LANG_SPEAK_DEFAULT);
    }, 10000);
    return () => {
      if (inactiveRef.current) window.clearTimeout(inactiveRef.current);
    };
  ...
  const startLevel = (level: CongKhoiLevel) => {
    setActiveLevel(level);
    setDeck(generateCongKhoiDeck(level));
    setQuestionIdx(0);
    setCorrectCount(0);
    setWrongInRound(0);
  ...
  const advanceQuestion = (earnedFully: boolean) => {
    ...
    if (questionIdx + 1 < deck.length) {
      ...
      if (finalCorrect >= CONGKHOI_PASS_THRESHOLD) {
  ...
  const tapOption = (n: number) => {
    ...
    if (n === q.correct) {
      ...
      window.setTimeout(() => {
        speak(`${q.a} cộng ${q.b} bằng ${q.correct}!`, LANG_SPEAK_DEFAULT);
      }, 350);
  ...
  // Confetti to khi pass level.
  useEffect(() => {
    if (phase !== 'finished' || !activeLevel) return;
    const passed = correctCount >= CONGKHOI_PASS_THRESHOLD;
    if (passed) {
      confetti({
        particleCount: 220,
        spread: 100,
      ...
      const t = window.setTimeout(() => {
        speak('Tuyệt vời! Bé biết cộng rồi!', LANG_SPEAK_DEFAULT);
      }, 400);
      return () => window.clearTimeout(t);
    }
    ...
    return (
      <div className="animate-in fade-in duration-500">
        ...
        <div className="text-center py-3 max-w-md mx-auto">
          {/* Hero animation: 2 stacks + result */}
          ...
          <h2 className="text-3xl font-black mb-2 bg-gradient-to-r from-orange-500 via-amber-500 to-sky-500 bg-clip-text text-transparent">
            Cộng Khối
          </h2>
          <p className="text-slate-500 text-sm max-w-xs mx-auto leading-relaxed mb-5">
            Hai tháp gặp nhau biến thành tháp lớn — bé đoán số mới!
          </p>

          <div className="space-y-3 text-left">
            {CONGKHOI_LEVELS.map((l) => {
              const passed = passedSet.has(l.id);
              const unlocked = isCongKhoiLevelUnlocked(l.id, passedSet);
              return (
                <button
                  key={l.id}
                  ...
                  className={`relative w-full p-4 rounded-3xl shadow-lg transition-all flex items-center gap-4 ${
                    !unlocked
                      ...
                      : passed
                        ? 'bg-gradient-to-br from-orange-100 via-amber-100 to-sky-100 border-2 border-amber-300 active:scale-95'
                        : 'bg-gradient-to-br from-orange-50 via-amber-50 to-sky-50 border-2 border-amber-200 active:scale-95'
                  }`}
                >
    ...
    return (
      <div className="text-center py-8 animate-in zoom-in duration-500 max-w-md mx-auto">
        ...
        <div className="flex justify-center items-end gap-1 mb-6 h-12">
          {Array.from({ length: QUESTIONS_PER_CONGKHOI_LEVEL }).map((_, i) => (
            <span
              key={i}
        ...
        <div className="bg-gradient-to-br from-orange-50 via-amber-50 to-sky-50 border-2 border-amber-200 rounded-3xl p-5 mb-6">
          <div className="text-5xl font-black bg-gradient-to-r from-orange-500 to-sky-500 bg-clip-text text-transparent">
            {correctCount}
            <span className="text-2xl text-slate-400">/{QUESTIONS_PER_CONGKHOI_LEVEL}</span>
          </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-4 bg-gradient-to-r from-orange-500 via-amber-500 to-sky-500 text-white rounded-2xl font-bold shadow-lg shadow-amber-200 active:scale-95 transition-all"
          >
            🔄 Chơi lại
          </button>
        </div>
  ...
  return (
    <div className="animate-in fade-in duration-300 max-w-md mx-auto">
      ...
      {/* Sàn xây dựng: 2 tháp hoặc 1 tháp merged */}
      <div className="bg-gradient-to-b from-sky-50 via-amber-50 to-orange-50 border-2 border-amber-200 rounded-3xl px-4 py-6 mb-4">
        <div className="flex justify-center items-end gap-2 min-h-[160px]">
          {!solved ? (
            // ── 2 tháp riêng + dấu cộng ──
            <>
              <div className="flex flex-col items-center gap-1">
                <Block value={q.a as BlockValue} />
              </div>
              <span className="text-4xl font-black text-slate-400 self-center">+</span>
              <div className="flex flex-col items-center gap-1">
                <Block value={q.b as BlockValue} />
              </div>
            </>
          ) : (
            // ── 1 tháp merged: b trên + a dưới ──
        ...
        <div className="text-center mt-3">
          <button
            type="button"
            onClick={() => speak(`${q.a} cộng ${q.b} bằng mấy?`, LANG_SPEAK_DEFAULT)}
            className="inline-flex items-center gap-1 px-3 py-1 bg-white/90 backdrop-blur border-2 border-amber-200 rounded-full active:scale-95 shadow-sm text-amber-700 font-bold text-xs"
          >
            🔁 Nghe lại
          </button>
        </div>
      ...
      <p className="text-center text-slate-500 text-xs font-bold mb-3">
        👆 Chạm số đúng — tổng 2 tháp là mấy?
      </p>
