...
const OBJECTS: PlusObject[] = [
  ...
  { id: 'car', emoji: '🚗', vi: 'xe đồ chơi' },
];

const TOTAL_ROUNDS = 10;
const HS_KEY = 'lingoland_plus_hs';
...
function buildQuestion(level: Level, prevObjectId?: string): Question {
  ...
  // a + b <= max, with both >= 1 so the problem is non-trivial
  const a = randInt(1, Math.max(1, max - 1));
  const b = randInt(1, Math.max(1, max - a));
  const answer = a + b;

  // 4 unique candidates around the answer, clamped to a sensible range
  const upper = Math.max(max, answer + 3);
  const pool = new Set<number>([answer]);
  let tries = 0;
  while (pool.size < 4 && tries < 40) {
    const cand = answer + randInt(-3, 3);
    if (cand >= 0 && cand <= upper && cand !== answer) pool.add(cand);
    tries++;
  }
  let backfill = 0;
  while (pool.size < 4 && backfill <= upper) {
...
export default function PlusView({ onBack }: PlusViewProps) {
    ...
    return (
      <div className="animate-in fade-in duration-500">
        ...
        <div className="text-center py-6 max-w-md mx-auto">
          ...
          <h2 className="text-3xl font-black mb-2 bg-gradient-to-r from-cyan-500 via-emerald-500 to-amber-500 bg-clip-text text-transparent">
            Phép cộng vui
          </h2>
          <p className="text-slate-500 text-sm max-w-xs mx-auto leading-relaxed mb-6">
            Học phép cộng bằng hình ảnh: thêm đồ vật vào nhóm và đếm tổng số.
          </p>

          <div className="bg-gradient-to-br from-cyan-50 via-emerald-50 to-amber-50 border-2 border-emerald-100 rounded-3xl p-5 mb-6">
            <div className="flex items-center justify-around mb-3">
              <span className="text-3xl">🍎🍎</span>
              <span className="text-2xl font-black text-emerald-500">+</span>
              <span className="text-3xl">🍎🍎🍎</span>
              <span className="text-2xl font-black text-emerald-500">=</span>
              <span className="text-3xl font-black text-amber-500 number-glow">5</span>
            </div>
    ...
    return (
      <div className="text-center py-8 animate-in zoom-in duration-500 max-w-md mx-auto">
        ...
        <p className="text-slate-500 text-sm mb-4">
          Bé đã giải xong {TOTAL_ROUNDS} câu phép cộng rồi!
        </p>
        ...
        <div className="bg-slate-50 rounded-3xl p-5 mb-3">
          <div className="text-5xl font-black bg-gradient-to-r from-cyan-500 via-emerald-500 to-amber-500 bg-clip-text text-transparent">
            {correctCount}
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={replay}
            className="flex-1 py-4 bg-gradient-to-r from-cyan-500 via-emerald-500 to-amber-500 text-white rounded-2xl font-bold shadow-lg shadow-emerald-200 active:scale-95 transition-all"
          >
            🔄 Chơi lại
          </button>
        </div>
  ...
  return (
    <div className="animate-in fade-in duration-300 max-w-3xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex items-center gap-3 text-xs font-black uppercase tracking-widest">
          <span className="text-slate-400">
            Câu <span className="text-emerald-500 text-base">{idx + 1}/{deck.length}</span>
          </span>
      ...
      <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden mb-4">
        <div
          className="h-full bg-gradient-to-r from-cyan-400 via-emerald-500 to-amber-500 transition-all duration-300"
      ...
      <div className="grid grid-cols-[1fr_auto_1fr_auto_1fr] items-stretch gap-1 md:gap-2 mb-5">
        <ObjectCard
          ...
          badge="bg-sky-500"
        />
        <CenterGlyph glyph="+" color="text-emerald-500" />
        <ObjectCard
          key={`b-${idx}`}
          count={question.b}
          emoji={question.object.emoji}
        ...
        />
        <CenterGlyph glyph="=" color="text-amber-500" />
        <ObjectCard
          key={`r-${idx}-${chosen !== null}`}
...
function ObjectCard({ count, emoji, accent, badge, revealed }: ObjectCardProps) {
  ...
  // Card 3 shows "?" until the user picks the correct answer
  if (revealed === false) {
    return (
      ...
      {count === 0 ? (
        ...
        <div className="flex flex-wrap justify-center items-center gap-1">
          {items.map((i) => (
            <span
              key={i}
              className="leading-none drop-in"
              style={{ fontSize, animationDelay: `${i * 0.05}s` }}
            >
              {emoji}
            </span>
          ))}
