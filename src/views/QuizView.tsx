...
import { LEVELS, type Level, type Word } from '../data/gameData';
...
function buildOptions(word: Word): string[] {
  ...
  while (opts.length < 4) {
    const r = all[Math.floor(Math.random() * all.length)].vi;
...
export default function QuizView({ level, onFinish }: QuizViewProps) {
  const { addScore } = useGame();
  const shuffledWords = useMemo(() => shuffle(level.words), [level]);
